...
const Login = ({ navigation }) => {
  ...
  useEffect(() => {
    ...
    /* const removeValue = async () => {
      try {
        await AsyncStorage.removeItem('user_dat')
      } catch(e) {
        // remove error
      }
    ...
    fetch(url, {
      ...
      if (data) {
        ...
        if (!data.success) {
          ...
          getUserData(`https://covidapptf.herokuapp.com/users/${data.user._id}`, data.user.token)
  ...
  const getUserData = (url, token) => {
    fetch(url, {
        ...
        if (data) {
            ...
            if (data.success) {
            /*   try {
                await AsyncStorage.setItem('user_contacts', data.user.contacts)
            } catch (e) {
                // save error
            } */
            } else {
              ...
              <ScrollView style={styles.container}>
                ...
                <SafeAreaView>
                  ...
                  <View style={styles.center}>
                    <Text style={{ marginVertical: 8 }}>Ainda não tens uma conta? <Text onPress={() => navigation.navigate('Register')} style={{ color: '#00C6AE' }}>Registra-te</Text></Text>
                    <Text style={{ color: '#00C6AE' }} onPress={() => navigation.navigate('RequestPasswordChange')}> Esqueci-me da palavra-passe</Text>
                  </View>
